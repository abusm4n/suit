...
# Device names to process (10 smart IoT devices from retrospective analysis)
DEVICES = [
    'amcrest-cam',
    'apple-tv',
    'd-link-mov',
...
def load_entropy_data(device_name):
    """Load entropy CSVs for a device from device folder, concatenating all files"""
    # Device folder with multiple CSV files (time-series captures)
    device_folder = os.path.join(ENTROPY_DIR, device_name)
    if os.path.isdir(device_folder):
        csv_files = sorted(list(Path(device_folder).glob("*.csv")))
        if csv_files:
            try:
                # Load and concatenate all CSVs for this device
                dfs = []
                ...
                return df, f"device folder ({len(csv_files)} files)"
...
def main():
    ...
    print("=" * 80)
    print("Retrospective Cross-Device Entropy Comparison Study - 10 Smart IoT Devices")
    print("=" * 80)
    
    # Step 1: Load and process data
    print("\n[1] Loading entropy data for all devices (10 devices from retrospective analysis)...")
    device_data = {}
    all_entropy = []
...
###############################################################################
    ...
    plt.savefig(os.path.join(FIGURES_DIR, "entropy_boxplots_re.pdf"), dpi=150, bbox_inches='tight')
    print(" Saved: entropy_boxplots_re.pdf")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURES_DIR, "entropy_distributions_re.pdf"), dpi=300, bbox_inches='tight')
    print("Saved: entropy_distributions_re.pdf")
    plt.close()
    ...
    with open(os.path.join(DATA_DIR, "entropy_analysis_report.txt"), "w") as f:
        f.write("=" * 80 + "\n")
        f.write("RETROSPECTIVE CROSS-DEVICE ENTROPY ANALYSIS REPORT - 10 SMART IOT DEVICES\n")
        f.write("=" * 80 + "\n\n")
        ...
        f.write("Figures:\n")
        f.write("  - entropy_boxplots_re.pdf\n")
        f.write("  - entropy_distributions_re.pdf\n")
        f.write("  - entropy_heatmap_re.pdf\n")
